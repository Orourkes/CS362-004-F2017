...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
	play_adventurer(drawntreasure, state, currentPlayer, z);
...
//      }
    ...
    case remodel:
	play_remodel(state, currentPlayer, choice1, choice2, handPos);
//      j = state->hand[currentPlayer][choice1];  //store card we will trash
//
...
/*********************************************************************
 * Student code, add in functions for cards
 * by Suzi O'Rourke
 * 10/8/17
 * 
 * *****************************************************************/
// Adventurer
int play_adventurer(int drawntreasure, struct gameState *state, int currentPlayer, int z)
{
      int cardDrawn, temphand[MAX_HAND];
      while(drawntreasure<=2){ //added logic error, adding =.
	if (state->deckCount[currentPlayer] <1){//if the deck is empty we need to shuffle discard and add to deck
	  shuffle(currentPlayer, state);
	}
	drawCard(currentPlayer, state);
...
{
	int i;
      for (i = 0; i <= 3; i++)	//one of the easiest errors is logic, in this case = added to for loop.
	{
	  drawCard(currentPlayer, state);
	}
      // card pile
...
int play_feast(struct gameState *state, int currentPlayer, int choice1)
{
      ...
      //Backup hand
      int i;
      for (i = 0; i < state->handCount[currentPlayer]; i++){	//logic issue, removed = in for loop.
	temphand[i] = state->hand[currentPlayer][i];//Backup card
	state->hand[currentPlayer][i] = -1;//Set to nothing
      }
      //Backup hand
...
int play_council_room(int currentPlayer, struct gameState *state, int handPos)
{
   int i;
     //+4 Cards
      for (i = 1; i < 4; i++)	//logic issue, starting at 1 instead of 0. Will get 3 cards instead of 4. 
	{
	  drawCard(currentPlayer, state);
	}
...
// Remodel
int play_remodel(struct gameState *state, int currentPlayer, int choice1, int choice2, int handPos)
{
      	int i, j;
	j = state->hand[currentPlayer][choice1];  //store card we will trash

      if ( (getCost(state->hand[currentPlayer][choice1]) + 2) >=  getCost(choice2) ) //error in logic, = added.
	{
	  return -1;
	}
